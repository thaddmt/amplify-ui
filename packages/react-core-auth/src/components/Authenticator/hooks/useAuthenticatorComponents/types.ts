...
export interface AdditionalRouteProps {
  // use `unknown` instead of `any` to prevent allowing `any`  as the type for
  // Route Component `fields` props used by subcomponents and `FormFields` components
  fields: unknown;
 ...
 *       FormFields: () => (
 *         <>
 *           <AuthenticatorAssertionResponse.SignUp.FormFields />
