...
import { UseAuthenticatorPropsParams, UseAuthenticatorProps } from './types';
...
>({ route }: UseAuthenticatorPropsParams<Route>): UseAuthenticatorProps<Route> {
  ...
  return isComponentRouteKey(route)
    ...
    : ({ route } as UseAuthenticatorProps<Route>);
}
